...
class EcommerceProductCrawler:
    ...
    async def fetch_url(self, session, url, visited_urls):
        """
        ...
        try:
            ...
            logger.error(f"Error fetching {url} SO !: {str(e)}")
    ...
    def save_results(self, output_file='product_urls.csv'):
        """
        ...
        for domain, urls in self.results.items():
            for url in urls:
                data.append({'domain': domain, 'product_url': url})
        ...
        for domain, urls in self.results.items():
            ...
            domain_file = f"products_{domain_name}.csv"
...
async def main():
    ...
    crawler = EcommerceProductCrawler(domains=domains)
